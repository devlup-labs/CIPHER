...
// test verification of the generated PoW for a request
func TestVerifyPoW(t *testing.T) {
	...
	proof := GeneratePoW(req, 2)

	if !VerifyPoW(req, proof) {
		t.Error("expected valid PoW to be accepted")
	}
}

func TestVerifyPoWRejectsModifiedRequest(t *testing.T) {
	req := model.Request{
		FileID:    "file-123",
		ClientID:  "client-456",
		Timestamp: 123456789,
	...
	req.FileID = "file-modified"

	if VerifyPoW(req, proof) {
		t.Error("expected PoW verification to fail for modified request")
	}
}
...
func TestVerifyPoWRejectsWrongNonce(t *testing.T) {
	...
	if VerifyPoW(req, proof) {
		t.Error("expected PoW verification to fail for wrong nonce")
	}
}
